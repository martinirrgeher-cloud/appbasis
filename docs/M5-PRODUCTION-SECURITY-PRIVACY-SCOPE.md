...
Der bestehende Ingress-Verifier bleibt unverändert. Seine accountweiten Worker-APIs und insbesondere das dem Worker zugeordnete `routes`-Inventar wurden bereits im Reference-Control-Plane-Slice bewusst gewählt, damit die Prüfung nicht von token-sichtbaren Zonen abhängt. M5 baut auf diesem bestehenden Security-Vertrag auf, statt eine zweite Route-Inventarisierung einzuführen.
