...
- jeder produktive Aufruf von `IdentityService.createInitialUser`, einschließlich vollständig abgeschlossener Retries, authentifiziert zuerst die konfigurierte Session als aktiven technischen Better-Auth-Admin, bevor Provisionierungs- oder Reconciliation-State gelesen wird
- zusätzlich bleibt die Backend-Autorisierung beim tatsächlichen Better-Auth-Create-/Existing-User-Pfad als Defense-in-Depth bestehen
...
Diese Invarianten werden zusätzlich in `packages/identity` real gegen PostgreSQL getestet; der Reference-E2E beweist die korrekte Composition dieser zentralen Garantien einschließlich eines bereits abgeschlossenen Bootstrap-Retry.
...
- ungültige, abgelaufene, deaktivierte oder nicht-administrative technische Sessions scheitern in der zentralen Identity-Runtime vor jeder zulässigen Provisionierungs-/Reconciliation-Auswertung, auch bei vollständig abgeschlossenen Retries
...
- gefälschte oder nicht-administrative Sessions können weder einen vorhandenen Better-Auth-User übernehmen noch einen bereits vollständig abgeschlossenen Bootstrap erfolgreich wiederholen
- ein gültiger Admin-Retry bleibt idempotent und ersetzt das ursprünglich ausgegebene temporäre Passwort nicht
...
- ein zweiter autorisierter Bootstrap desselben Usernames liefert dieselbe Identity-ID
