...
const disabledUsername = 'disabled.existing';
const disabledPassword = 'Disabled-existing-password-42!';
...
describe('Reference demo user bootstrap PostgreSQL E2E', () => {
  beforeAll(async () => {
    ...
    await auth.api.createUser({
      body: {
        ...
        role: 'admin',
        data: { username: otherAdminUsername, displayUsername: otherAdminUsername },
      },
    });
    await auth.api.createUser({
      body: {
        ...
        role: 'user',
        data: { username: preexistingUsername, displayUsername: preexistingUsername },
      },
    });
    await auth.api.createUser({
      body: {
        email: 'disabled-existing@identity.invalid',
        password: disabledPassword,
        name: 'Disabled Existing User',
        role: 'user',
        data: { username: disabledUsername, displayUsername: disabledUsername },
      },
    });
    await migrationConnection.client`
      UPDATE "user" SET banned = true WHERE username = ${disabledUsername}
    `;

    administrativeSessionToken = await signInSession(auth, adminUsername, adminPassword);
    nonAdministrativeSessionToken = await signInSession(auth, preexistingUsername, preexistingPassword);
  ...
  it('delegates existing-user authorization to the hardened Identity runtime', async () => {
  ...
  it('delegates technical-admin target protection to the hardened Identity runtime', async () => {
  ...
  it('provisions one identity, reauthorizes completed retries and never replaces the temporary password', async () => {
    ...
    for (const invalidAdministrativeSessionToken of [
      'better-auth.session_token=forged-completed-retry',
      nonAdministrativeSessionToken,
    ]) {
      await expect(
        bootstrapReferenceDemoUser({
