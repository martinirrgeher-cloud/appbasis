...
test("requires the exact preview target and explicit apply gate", () => {
  assert.throws(
    () =>
      readReferenceRootAdminEnvironment({
        ...valid,
        APPBASIS_ROOT_ADMIN_TARGET: "production",
      }),
    ReferenceRootAdminEnvironmentError,
  );
  assert.throws(
    () =>
      readReferenceRootAdminEnvironment({
        ...valid,
        APPBASIS_ROOT_ADMIN_APPLY: "0",
      }),
    ReferenceRootAdminEnvironmentError,
  );
});

test("requires every runtime input without normalizing the secret value", () => {
  for (const name of [
  ...
  assert.equal(
    readReferenceRootAdminEnvironment(valid).password,
    valid.APPBASIS_ROOT_ADMIN_PASSWORD,
  );
});
