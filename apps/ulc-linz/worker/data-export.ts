...
/**
 * Narrow app-owned authorization proof. Its adapter must resolve active ULC
 * membership and, for self/managed exports, the exact canonical subject
 * relation before returning. The coordinator validates that the proof matches
 * the request before any dataset is read.
 */
...
function assertAuthorizationMatches(
  ...
  // Self-export is a privacy operation for any authenticated ULC role. The
  // authorization adapter must prove the explicit self relation; this layer
  // does not infer self from the role name.
