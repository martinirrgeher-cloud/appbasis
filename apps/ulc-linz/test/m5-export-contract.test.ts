...
describe("ULC Linz M5-E export contract", () => {
  ...
  it("exports only current member/contact master data as a normal user dataset", () => {
    ...
    expect(exportContract.datasets[0]?.sourceTables.map(tableKey)).toEqual([
      "identity:appbasis_person",
      "identity:user",
    ]);
  });

  it("keeps credential, authorization and security tables outside normal export", () => {
