  ...
  return {
    ...
    memberships: {
      async resolveMembership() {
        return { organizationId: ORGANIZATION_ID, sourceRole: "athlete", active: true };
      },
    },
    subjectScopes: { async hasRelation() { return true; } },
    async readDatasets() {
      return [{
        id: "member-contact",
        records: [{
...
describe("ULC Linz M5-E fail-closed resolver boundaries", () => {
  it("does not read datasets when membership resolution fails", async () => {
    ...
    const dependencies: UlcLinzCanonicalDataExportDependencies = {
      ...base,
      memberships: { async resolveMembership() { throw new Error("membership unavailable"); } },
      readDatasets,
    };
    await expect(exportUlcLinzDataWithCanonicalAuthorization(currentIdentity(), dependencies, REQUEST)).rejects.toThrow("membership unavailable");
  ...
  it("does not read datasets when self relation resolution fails", async () => {
    ...
    const dependencies: UlcLinzCanonicalDataExportDependencies = {
      ...base,
      subjectScopes: { async hasRelation() { throw new Error("subject scope unavailable"); } },
      readDatasets,
    };
    await expect(exportUlcLinzDataWithCanonicalAuthorization(currentIdentity(), dependencies, REQUEST)).rejects.toThrow("subject scope unavailable");
    expect(readDatasets).not.toHaveBeenCalled();
  });
  ...
  it("blocks an assigned but inactive or missing runtime role before dataset read", async () => {
    ...
    await expect(exportUlcLinzDataWithCanonicalAuthorization(currentIdentity(), dependencies, REQUEST)).rejects.toMatchObject({ code: "AUTHORIZATION_MISMATCH" });
    expect(readDatasets).not.toHaveBeenCalled();
  });
  ...
  it("does not audit or return a partial export when dataset reading fails", async () => {
    ...
    const dependencies: UlcLinzCanonicalDataExportDependencies = {
      ...base,
      async readDatasets() { throw new Error("dataset read failed"); },
      recordExportAudit,
    };
    await expect(exportUlcLinzDataWithCanonicalAuthorization(currentIdentity(), dependencies, REQUEST)).rejects.toThrow("dataset read failed");
