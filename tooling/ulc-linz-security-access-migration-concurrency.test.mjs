...
test("destructive retention gate enforces the complete non-owner audit ACL set", async () => {
  ...
  assert.match(source, /CROSS JOIN pg_catalog\.pg_roles backup_role/);
  ...
  assert.match(source, /backup_role\.rolname = \$1/);
  ...
  assert.match(source, /acl\.grantee = backup_role\.oid/);
