...
function successfulSteps({ allowedPasswordChange = false } = {}) {
  const steps = [
    {
      path: "/api/auth/sign-in",
      method: "POST",
      response: () =>
        jsonResponse(session("m3.smoke.denied", "denied-id"), {
          cookie: "session=denied",
        }),
    },
    {
      ...
      },
      {
        ...
        response: () =>
          jsonResponse(
            ...
            { cookie: "session=allowed-temporary" },
          ),
      },
      {
        ...
        method: "POST",
        response: () =>
          jsonResponse(session("m3.smoke.allowed", "allowed-id"), {
            cookie: "session=allowed-final",
          }),
      },
    );
  } else {
...
test("replaces a bootstrap temporary password before running the allowed smoke", async () => {
