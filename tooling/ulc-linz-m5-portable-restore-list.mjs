...
export function filterExactProviderDefaultAclToc(tocText) {
  ...
  const expectedKinds = EXPECTED_PROVIDER_DEFAULT_ACLS.map((entry) => entry.tocKind).sort();
  if (JSON.stringify(observed.sort()) !== JSON.stringify(expectedKinds)) {
...
function compareDefaultAcl(left, right) {
  return `${left.ownerRole}\u0000${left.schemaName ?? ""}\u0000${left.objectType}\u0000${left.acl}`.localeCompare(
    `${right.ownerRole}\u0000${right.schemaName ?? ""}\u0000${right.objectType}\u0000${right.acl}`,
  );
}
