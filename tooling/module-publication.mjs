...
const REGISTRY_LOCK_FILE = ".appbasis-module-registry.lock";
const LOCK_CANDIDATE_PREFIX = ".appbasis-module-registry-candidate-";
const DEFAULT_WAIT_TIMEOUT_MS = 120_000;
const RETRY_DELAY_MS = 20;

export async function acquireModuleRegistryLock(
  repositoryRoot,
  purpose,
  options = {},
) {
  if (purpose !== "publish" && purpose !== "verify") {
    throw new Error(`Invalid module registry lock purpose: ${purpose}.`);
  }
  ...
  while (true) {
    ...
    try {
      ...
      if (state.kind === "invalid") {
        throw new Error("Invalid module registry lock requires cleanup.");
      }
      if (state.kind === "stale") {
        throw new Error("Stale module registry lock requires cleanup.");
      }
      if (Date.now() >= deadline) {
        throw new Error("Timed out waiting for active module registry lock.");
      }
...
export async function withModuleRegistryLock(
  repositoryRoot,
  purpose,
  operation,
  options = {},
) {
  const claim = await acquireModuleRegistryLock(
    repositoryRoot,
    purpose,
    options,
  );
