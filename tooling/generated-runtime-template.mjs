...
const GENERATED_WORKER_PATH = "worker/index.ts";
const POSTGRES_RUNTIME_PATH = "worker/postgres.ts";
...
export function createIdentityRuntimeTemplate(input) {
  ...
  const filesWithPreview = withGeneratedPreviewEntrypoint(files, generated.appId);

  return Object.freeze({
    ...generated,
    files: Object.freeze(filesWithPreview),
  });
}

function withGeneratedPreviewEntrypoint(files, appId) {
  const hasGeneratedPostgresWorker =
    files.some((entry) => entry.path === GENERATED_WORKER_PATH) &&
    files.some((entry) => entry.path === POSTGRES_RUNTIME_PATH);
  if (!hasGeneratedPostgresWorker) return files;

  if (files.some((entry) => entry.path === PREVIEW_ENTRYPOINT_PATH)) {
    throw new Error("Generated runtime already contains a preview entrypoint.");
  }
  ...
  return [
    ...
    Object.freeze({
      ...
      content: generatedPreviewEntrypoint(appId),
    }),
  ];
}
